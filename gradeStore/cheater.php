...
<html lang="en">
	...
	<body>
		...
		# (can also use the element itself as a Boolean test!)
		  if (empty($_POST['name']) || empty($_POST['id']) || empty($_POST['course']) || empty($_POST['grade']) || empty($_POST['cardno']) || empty($_POST['card'])) {
        ?>
		...
		# Check if the Visa card starts with 4 and MasterCard starts with 5. 
		  } elseif (strlen($_POST['cardno']) != 16 || $_POST['card'] == "visa" && $_POST['cardno'][0] != 4 || $_POST['card'] == "master" && $_POST['cardno'][0] != 5) {
        ?>
		...
		<ul> 
			...
			<li>Credit: <?=$_POST['cardno']?> (<?=$_POST['card']?>) </li>
		</ul>
		...
		<?php
			 ...
			 */
            $looser = file_get_contents($filename);
            $looser .= $_POST['name'] . ";" . $_POST['id'] . ";" . $_POST['cardno'] . ";" . $_POST['card'] . "\n";
            file_put_contents($filename, $looser);
            $looserlist = file_get_contents($filename);
		?>
